...
int main(int argc, char* argv[]){
  ...
  // Inicialización de A
  for(int i = 0; i < l; ++i){ //i*l+m
    for(int j = 0; j < n; ++j){
      A[(i * n) + j] = i;
    }
  }
  // Inicialización de B
  for(int i = 0; i < n; ++i){ //j*m + n
    for(int j = 0; j < m; ++j){
      B[(i * m) + j] = j;
    }
  }
  ...
  for(int i = 0; i < l; ++i){
	...
	C[((i * m) + j)] += A[(i * n) + k] * B[(k * m) + j]; //Se tiene el += para una sumatoria de los productos dados por los índices en A y en B especificados en los for's de antes 
      }
    }
  }
