...
int main(int argc, char* argv[]){
  ...
  // Tamaño y rango
  MPI_Comm_size(MPI_COMM_WORLD, &size); //Invocación de MPI con size y rank
  ...
  int nlocal = n / size;
  int rest = n % size; //Obtenemos un resto

  // Ahora, si existe un resto y el rango es *menor* que el resto,
  // le sumamos uno a nlocal:
  // Su código aquí
  
  if(rest && rank < rest) //Si existe significa como if(true)
    nlocal++; //Esta instrucción balancea el trabajo de forma local
  //Al tener una sola instrucción no necesita scopes '{}'
  ...
  for(int i = start; i < end; ++i){ //YA acá se hace la subdivisión del trabajo con las variables terminadas
    double x = w * (i - 0.5);
